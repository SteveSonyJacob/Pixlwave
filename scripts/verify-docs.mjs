...
const discoveryContracts = await fs.readFile(path.join("docs", "architecture", "P03-discovery-communication-contracts.md"), "utf8");
...
const requiredDiscoveryContracts = ["quote_snapshots", "reserves no inventory", "private internal notes", "does not claim observed playback", "Supabase auth SMTP"];
for (const phrase of requiredDiscoveryContracts) if (!discoveryContracts.includes(phrase)) throw new Error(`P03 contract is missing: ${phrase}`);
...
console.log("Documentation contract checks passed: R01-R18, D01-D17, P01/P02/P03 policy examples, and obsolete-route absence.");
