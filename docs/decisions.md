...
Version: 1.0 implementation draft. Updated: 2026-09-17.
...
| D09 | PARTIAL / GOOGLE + RESEND SELECTED | Supabase authentication; in-app/email events and support tickets. Verified email/password is primary, Google OAuth is the selected alternate sign-in, and Resend is selected for Auth SMTP plus application email API delivery. Normal Gmail/other recipient delivery allowed. Requests go to admin, not automatically to owner. | Google OAuth and Resend Auth SMTP are verified. Configure a least-privilege Resend API key, verified application sender and signed delivery webhook; validate real application delivery/bounce behavior. Phone OTP is deferred unless later selected with a TRAI DLT-compliant provider. Send owners only the confirmed fulfillment information admin authorizes, never raw request queues/private coordination notes. | P01, P03 |
...
| D14 | SERVICE FIELDS CONFIRMED / MEDIA FOUNDATION IMPLEMENTED | Owners specify ad duration, plays per show/day and operating hours per listing, subject to admin approval. P02 uses private PDF/PNG/JPEG owner documents up to 10 MB and PNG/JPEG/MP4/WebM creatives up to 50 MB, with 180/365-day initial retention, signature/EICAR checks and five-minute signed downloads. | P02 validates category capacity/service promises and approved snapshots. P04/P05 must copy and rerun committed terms. Production malware scanning, video metadata probing and final legal retention approval remain launch gates; the built-in check is not represented as a full scanner. Owners fulfill service outside the website. | P02, P04, P06-P07 |
...
| 2026-09-17 | 1.0 implementation draft | Recorded P02 media engineering limits, private access expiry and provisional retention. No business policy changed; production scanner/video probe and client retention approval remain open. |
