...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (asset.error || !asset.data || asset.data.purpose !== "creative") return Response.json({ error: "Creative not found." }, { status: 404 });
  if (asset.data.scan_status !== "clean") return Response.json({ error: "Creative remains quarantined." }, { status: 409 });
