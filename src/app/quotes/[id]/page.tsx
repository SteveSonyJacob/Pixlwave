...
export default async function QuotePage({ params }: PageProps) {
  ...
  return <main className="dashboard-shell quote-page"><div className="dashboard-heading"><div><span className="eyebrow">Immutable price snapshot</span><h1>{snapshot.title ?? "Media quote"}</h1><p>{snapshot.locality}, {snapshot.district}</p></div><span className={`status ${current ? "status-ok" : "status-danger"}`}>{current ? "current" : "refresh required"}</span></div>
    <section className="panel-card quote-summary"><div><span>Quoted total</span><strong>{formatInr(quote.total_amount_paise)}</strong><small>{quote.quantity} × {formatInr(quote.unit_amount_paise)} / {snapshot.rateUnit?.replaceAll("_", " ")}</small></div><dl><div><dt>Created</dt><dd>{formatIst(quote.created_at)}</dd></div><div><dt>Expires</dt><dd>{formatIst(quote.expires_at)}</dd></div><div><dt>Rate revision</dt><dd>{quote.rate_revision_id.slice(0, 8)}</dd></div></dl></section>
