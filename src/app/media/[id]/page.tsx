  ...
  return <main className="dashboard-shell wide-dashboard media-detail">
    ...
    <div className="media-hero">
      ...
      <aside className="quote-card panel-card"><span className="eyebrow">Published rate</span><h2>{formatInr(listing.amount_paise)}</h2><p>per {listing.rate_unit.replaceAll("_", " ")} · rate effective {new Date(listing.rate_effective_at).toLocaleDateString("en-IN")}</p><MessageBanner error={query.error} /><form action={createQuote} className="form-stack"><input type="hidden" name="listingId" value={id} /><input type="hidden" name="category" value={listing.category} />
        {listing.category === "theatre" ? <><label>Shows (select one or more)<select name="showInstanceIds" required multiple size={Math.min(6, Math.max(2, shows.length))}>{shows.map((show) => <option key={show.id} value={show.id}>{new Date(show.starts_at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Kolkata" })} · {show.slots_total} published slots</option>)}</select></label><label>Slots per selected show<input name="quantity" type="number" min="1" max="50" defaultValue="1" required /></label></> : <><div className="two-fields"><label>Start date<input name="startDate" type="date" min={new Date().toISOString().slice(0, 10)} required /></label><label>End date (inclusive)<input name="endDate" type="date" min={new Date().toISOString().slice(0, 10)} required /></label></div>{listing.category === "mobile" ? <label>Rotating slots per date<input name="quantity" type="number" min="1" max={Number(details.rotatingSlots ?? 30)} defaultValue="1" required /></label> : null}</>}
