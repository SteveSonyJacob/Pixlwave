...
export default async function CartPage({ searchParams }: PageProps) {
  ...
  return <main className="dashboard-shell wide-dashboard booking-page">
    <div className="dashboard-heading"><div><span className="eyebrow">Frozen-cart checkout</span><h1>Your booking carts</h1><p>Add current quotes to one open cart. Submission freezes membership; later additions automatically use a new cart.</p></div><Link className="button button-secondary button-small" href="/discover">Add placement</Link></div>
    <MessageBanner message={query.message} error={query.error} />
    <div className="banner banner-warning">Razorpay is intentionally not connected in Phase 4. Submitted carts cannot be paid from the browser; only the restricted test-database fixture can create paid review records.</div>
