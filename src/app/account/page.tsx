...
export default async function AccountPage({ searchParams }: PageProps) {
  ...
  return <main className="dashboard-shell">
    <div className="dashboard-heading"><div><span className="eyebrow">Account foundation</span><h1>Hello, {identity.fullName.split(" ")[0]}</h1><p>{identity.isAdmin ? "Manage your profile and open the admin console." : "Manage one verified identity and move between your advertiser and owner workspaces."}</p></div><form action={signOut}><button className="button button-secondary button-small">Sign out</button></form></div>
    <MessageBanner message={query.message} error={query.error} />
    {identity.isAdmin ? (
      <section className="mode-switcher">
        <div><span className="eyebrow eyebrow-light">Administrator access</span><h2>Admin workspace</h2><p>Open the admin console to review platform activity. Authenticator verification is required.</p></div>
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel-card"><div className="panel-title"><div><span className="eyebrow">Access summary</span><h2>What you can open</h2></div></div><div className="access-list">{identity.isAdmin ? null : <><Link href="/advertiser"><b>Advertiser dashboard</b><span>Enabled →</span></Link><Link href="/owner"><b>Owner dashboard</b><span>Enabled →</span></Link></>}<Link href="/support"><b>Support tickets</b><span>Open →</span></Link><Link href="/notifications"><b>Notifications</b><span>View →</span></Link><Link href="/account/security"><b>Security & MFA</b><span>Manage →</span></Link>{identity.isAdmin ? <Link href="/admin"><b>Admin console</b><span>MFA required →</span></Link> : <div><b>Admin console</b><span>Not granted</span></div>}</div></section>
    </div>
