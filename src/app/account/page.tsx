...
export default async function AccountPage({ searchParams }: PageProps) {
  ...
  return <main className="dashboard-shell">
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel-card"><div className="panel-title"><div><span className="eyebrow">Access summary</span><h2>What you can open</h2></div></div><div className="access-list"><Link href="/advertiser"><b>Advertiser dashboard</b><span>Enabled →</span></Link><Link href="/owner"><b>Owner dashboard</b><span>Enabled →</span></Link><Link href="/account/security"><b>Security & MFA</b><span>Manage →</span></Link>{identity.isAdmin ? <Link href="/admin"><b>Admin console</b><span>MFA required →</span></Link> : <div><b>Admin console</b><span>Not granted</span></div>}</div></section>
    </div>
    <IdentifierLinking hasEmail={Boolean(identity.email)} hasPhone={Boolean(identity.phone)} />
  </main>;
}
