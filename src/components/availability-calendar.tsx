...
export function AvailabilityCalendar({ category, blackouts, shows }: { category: "led" | "theatre" | "mobile"; blackouts: Blackout[]; shows: Show[] }) {
  ...
  return <section className="detail-section availability-calendar"><h2>Published availability</h2><p>Availability is provisional until an administrator confirms a later paid request. Blackouts and published show instances are shown in IST.</p><div className="calendar-legend"><span><i className="calendar-open" />Available to quote</span><span><i className="calendar-blocked" />Unavailable</span>{category === "theatre" ? <span><i className="calendar-show" />Published show</span> : null}</div><div className="calendar-grid" role="grid" aria-label="Next 28 days of published availability">{dates.map((date) => {
    ...
    const reason = blackout ? blackout.reason : category === "theatre" && showsOnDay === 0 ? "No published show" : category === "theatre" ? `${showsOnDay} published ${showsOnDay === 1 ? "show" : "shows"}` : "Available to quote";
