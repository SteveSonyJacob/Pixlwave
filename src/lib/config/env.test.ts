...
describe("configuration boundary", () => {
  ...
  it("fails clearly for missing values and non-India main data", () => {
    const result = inspectServerEnv({ ...valid, DATABASE_REGION: "eu-west-1", AUTH_SMS_PROVIDER: "change-me", AUTH_OAUTH_PROVIDERS: "apple" });
    expect(result.valid).toBe(false);
    if (!result.valid) expect(result.errors.map((item) => item.field)).toEqual(expect.arrayContaining(["DATABASE_REGION", "AUTH_SMS_PROVIDER", "AUTH_OAUTH_PROVIDERS"]));
  });
});
